...
def age_from_sec(second):
    years = second // (60 * 60 * 24 * 365) 
    second -= years * (60 * 60 * 24 * 365)
...
def accurate_age(b_year, b_month, b_day):
    ...
    actual_month = 7
    actual_day = 18
    actual_year_to_sec = (actual_year - 1900) * 365 * 24 * 60 * 60
    actual_month_to_sec = (actual_month - 1) * 30 * 24 * 60 * 60
    actual_day_to_sec = (actual_day - 1) * 24 * 60 * 60

    birtday_year_to_sec = (b_year - 1900) * 365 * 24 * 60 * 60
    birtday_month_to_sec = (b_month - 1) * 30 * 24 * 60 * 60
    birthday_day_to_sec = (b_day - 1) * 24 * 60 * 60

    actual_date_in_sec = actual_year_to_sec + actual_month_to_sec + actual_day_to_sec
    bday_date_in_sec = birtday_year_to_sec + birtday_month_to_sec + birthday_day_to_sec

    actual_age_in_sec = actual_date_in_sec - bday_date_in_sec
